...
namespace libff
{
    ...
    template <mp_size_t_ n>
    __device__ unsigned char* bigint<n>::getByteChunks() const
    {
        unsigned char* dexp = (unsigned char*) &this->data[0];
        return dexp;
    }
    template <mp_size_t_ n>
    __device__ bigint<n> bigint<n>::operator<<(int shift) const
    {
        bigint<n> shifted("0");
        int block_count = shift/(sizeof(mp_size_t_)*8);
        int filtersize = shift - (block_count* sizeof(mp_size_t_)*8);
        int revFilter = sizeof(mp_size_t_)*8 - filtersize;
        assert(block_count*sizeof(mp_size_t_)*8 + filtersize == shift);

        uint64_t one = 1;
        uint64_t mask = (one<<revFilter)-1;
        mask = 0xFFFFFFFFFFFFFFFF - mask;
        for(int i= n-block_count-1;i >= 0; i--)
        {
            shifted.data[i+block_count] = data[i];
        }
        ...
        for(int i=n-1; i > 0; i--)
        {
            shifted.data[i] = shifted.data[i]<<filtersize;
            shifted.data[i] = shifted.data[i] | ((shifted.data[i-1] & mask)>>revFilter);
        }
        shifted.data[0] = shifted.data[0]<<filtersize;
        return shifted;
    }
    template <mp_size_t_ n>
    __device__ bigint<n> bigint<n>::operator=(const bigint<n> bignumber) const
    {
        for(int i=0; i<n;i++)
            data[i] = bignumber.data[i];
        return *this;
    }
}
