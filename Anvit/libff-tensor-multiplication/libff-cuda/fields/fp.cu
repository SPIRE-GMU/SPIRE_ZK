...
#include "../common/multiplication-tensor.cuh"
// #include "../mini-mp-cuda/mini-mp-cuda.cu"
...
namespace libff
{
    ...
    template <mp_size_t_ n>
    __device__ void Fp_model<n>::set_mont_representation(unsigned char *new_mont_repr, int new_size)
    {

        int old_size = (this->mont_repr.num_bits() + 7) / 8;
        unsigned char *q = (unsigned char *)&this->mont_repr.data[0];

        bigint<n + n + 1> num1("0");
        bigint<n + n + 1> num2("0");
        bigint<n + n + 1> modulus("0");

        unsigned char *m = (unsigned char *)(&num1.data[0]);
        unsigned char *p = (unsigned char *)(&num2.data[0]);

        num1.clear();
        num2.clear();
        int blocks = (new_size + sizeof(mp_limb_t_) - 1) / (sizeof(mp_limb_t_));
        for (int i = 0; i < blocks * sizeof(mp_limb_t_); i++)
        {
            m[i] = new_mont_repr[i];
        }

        for (int i = 0; i < n; i++)
            modulus.data[i] = (*params).modulus->data[i];
        num2 = modulus;

        size_t modlen = (*params).modulus->num_bits();
        size_t numlen = num1.num_bits();

        while (num1 >= num2)
        {
            numlen = num1.num_bits();
            // printf("num: %lu, mod: %lu\n", numlen, modlen);
            int toshift = (numlen - modlen - 1);
            toshift = toshift > 0 ? toshift : 0;
            num2 = (modulus << toshift);
            // printf("Mod with:\n");
            // num2.print();
            while (num1 >= num2)
            {
                // printf("num1 : %lu, num2: %lu\n", num1.num_bits(), num2.num_bits());
                // num1.print();
                num1 -= num2;
            }
        ...
        for (int i = 0; i < n; i++)
        {
            mont_repr.data[i] = num1.data[i];
        }
    }

    template <mp_size_t_ n>
    __device__ int Fp_model<n>::getDataSize(unsigned char *res)
    {
        int size = (n + n) * 8 + 2;
        int last_index = 0;
    ...
    {
        uint8_t *num1 = this->mont_repr.getByteChunks();
        uint8_t *num2 = other.getByteChunks();
        uint8_t *res = getMultiplicationResult(num1, num2, 8 * n * sizeof(mp_limb_t_), 8 * n * sizeof(mp_limb_t_));

        int size = this->getDataSize(res);

        __syncthreads();
        __syncwarp();
        cudaDeviceSynchronize();
        this->set_mont_representation(res, size);
        if (mont_repr >= *params->modulus)
            mont_repr -= *params->modulus;

        mp_limb_t_ t[n + 2] = {0};
        for (int i = 0; i < n; i++)
            t[i] = mont_repr.data[i];
