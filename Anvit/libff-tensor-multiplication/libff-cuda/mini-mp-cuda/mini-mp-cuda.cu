#ifndef MINI_MP_CUDA_H
#define MINI_MP_CUDA_H
...
/* GCD */
__device__ static mp_limb_t_ my_mpn_gcd_11(mp_limb_t_ u, mp_limb_t_ v)
{
    unsigned shift;
...
__device__ unsigned long mpz_gcd__ui_(mpz_t_ g, const mpz_t_ u, unsigned long v)
{
    ...
    else
    {
        un = GMP_ABS(u->_mp_size);
        if (un != 0)
            v = my_mpn_gcd_11(mpn_div_qr_1(NULL, u->_mp_d, un, v), v);
    ...
    if (tu->_mp_size == 0)
    {
        ...
        for (;;)
        {
            ...
            if (tv->_mp_size == 1)
            {
                ...
                mpz_set__ui_(g, my_mpn_gcd_11(ul, vl));
                break;
            }
            mpz_sub_(tu, tu, tv);
        }
