...
// __global__ void init_ell_matrix(ELL_matrix_opt *ell, size_t total, size_t tnum, size_t max_length, size_t window_size, size_t num_groups)
// {
//     for (int i = 0; i < num_groups; i++)
//     {
//         ell[i].init(max_length, tnum, (size_t)1 << window_size);
//     }
// }
...
//     CUDA_CHECK(cudaMalloc(&ell_matrix, sizeof(ELL_matrix_opt) * num_groups));
//     this->tnum = gridSize * blockSize;
//     this->max_length = (total + tnum - 1) / tnum;
//     init_ell_matrix<<<1, 1>>>(ell_matrix, total, tnum, max_length, window_size, num_groups);
...
//         Vector<G1Point> temp_buckets(1 << window_size), temp_res(group_grid * blockSize);
...
//         construct_ell_matrix<<<gridSize, blockSize>>>(scalars, &ell_matrix[i], i, total_num, window_size);
//         CUDA_CHECK(cudaDeviceSynchronize());
//         printf("Constructed ell matrix for group %d\n", i);
//         transpose_ell2csr(&ell_matrix[i], csr_matrix);
//         CUDA_CHECK(cudaDeviceSynchronize());
...
// {
//     printf("ELL Matrix details:\n");
//     printf("row_size: %lu, col_size: %lu\n", mtx->row_size, mtx->col_size);
//     pres->col_size = mtx->row_size;
//     pres->row_size = mtx->col_size;
//     *pizero = 0;
//     pres->row_ptr.resize_fill(mtx->col_size + 1, *pizero);
//     col_idx_data_addr a;
//     a.col_idx = 0;
//     a.data_addr = 0;
//     pres->col_data.resize_fill(mtx->total, a); 
...
//     }
// }
// __global__ void update_csr_row_ptr2(CSR_matrix_opt *pres, size_t level, size_t stride)
// {
...
//     }
// }
// __global__ void update_col_data(ELL_matrix_opt *mtx, CSR_matrix_opt *pres, Vector<size_t> *p_offset)
// {
//     size_t tid = threadIdx.x + blockDim.x * blockIdx.x;
//     size_t t_num = blockDim.x * gridDim.x;
//     while (tid < mtx->row_size)
...
//     if (t != cudaSuccess)
//         printf("Error: %s, File = %s, Line = %d\n", cudaGetErrorName(t), __FILE__, __LINE__);
...
//     launch<<<1, 1>>>([=] __device__(Vector<size_t> * p_offset, ELL_matrix_opt * mtx)
//                     {
//                         size_t zero = 0; 
//                         p_offset->resize_fill(mtx->col_idx.size(), zero); 
//                     }, p_offset, mtx);
...
//     {
//         update_csr_row_ptr2<<<gridSize / 2, blockSize>>>(pres, i, stride);
//         CUDA_CHECK(cudaDeviceSynchronize());
//         CUDA_CHECK(cudaStreamSynchronize(0));
//     }
...
//     update_col_data<<<gridSize, blockSize>>>(mtx, pres, p_offset);
//     CUDA_CHECK(cudaDeviceSynchronize());
//     CUDA_CHECK(cudaStreamSynchronize(0));
//     printf("Transposition complete\n");
// }

// __global__ void spmv_small_rows(size_t z, CSR_matrix_opt *mtx, G1Point *vec, Vector<G1Point> *res)
// {
...
//     spmv_small_rows<<<gridSize, blockSize>>>(z, mtx, points, res);
//     CUDA_CHECK(cudaDeviceSynchronize());
