...
__global__ void process_scalar_1(uint16_t *scalar, uint32_t *scalar_tuple,
...
__global__ void bucket_agg_2(bucket_t *buckets);

__global__ void recursive_sum(bucket_t *buckets, bucket_t *res);
                        ...
                        Types... args)
{
    ...
    // printf("Launching with Grid.X = %lu, grid.Y = %lu, block = %lu, stream = %d\n", gridDim.x, gridDim.y, blockDim.x, stream);
